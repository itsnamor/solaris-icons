...
import * as solaris from "@solaris-icons/react";
...
const DEFAULT_GROUP_ICONS: GroupIcon[] = Object.entries(
  groupBy(metadata, (metadata) => metadata.category),
).map(([category, iconsMetadata]) => ({
  category,
  iconsMetadata,
}));
...
export function IconsExplorer() {
  const [displayIcons, setDisplayIcons] =
    useState<GroupIcon[]>(DEFAULT_GROUP_ICONS);

  const [variant, setVariant] =
    useState<(typeof variants)[number]>("bold-duotone");
  ...
  const handleSearch = useRef(
    debounce(async (value) => {
      ...
      const groupedIcons = Object.entries(
        groupBy(
  ...
  return (
    <Surface className="p-4 m-4 rounded-lg h-[calc(100vh-90px)] flex flex-col gap-4 overflow-hidden">
      <div className="mb-4 flex gap-4 items-center">
        <Input
          className="h-10 w-80"
          onChange={(e) => handleSearch.current(e.target.value)}
        ...
        <RadioGroup
          ...
          value={variant}
          onChange={(e) => setVariant(e as (typeof variants)[number])}
        >
          ...
          {displayIcons.map(({ category, iconsMetadata }) => (
            <div key={category} className="flex flex-col gap-2 mb-10">
              <div className="font-semibold mb-2">
                {category} ({iconsMetadata.length})
              </div>

              <div className="flex flex-wrap gap-8">
                {iconsMetadata.map(({ icon }) => {
                  const iconName = `Icon${startCase(icon).replace(/ /g, "")}`;
                  const Icon = solaris[iconName as keyof typeof solaris];

                  return (
                    <div
                      key={icon}
                      className="size-12 flex flex-col items-center"
                    >
                      <Tooltip delay={0}>
                        <Tooltip.Trigger>
